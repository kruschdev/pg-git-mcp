...
3. **Temporal Decay**: The `pg_git_semantic_search` tool dynamically calculates cosine similarity using the `pgvector` HNSW index, and explicitly multiplies the score by an exponential time decay `exp(-0.01 * age)` using the `last_seen_at` timestamp.
